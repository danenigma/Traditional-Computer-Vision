...
def main(args):
	...
	new_classifier = nn.Sequential(nn.Dropout(p=0.5),
    							   nn.Conv2d(512, num_classes, kernel_size=(1, 1), stride=(1, 1)),
    							   nn.ReLU(),
    							   nn.AvgPool2d(kernel_size=13, stride=1, padding=0, ceil_mode=False, count_include_pad=True))
    								
	model.classifier = new_classifier
	print(model)
	X = torch.rand(2, 3, 224, 224)
	print(model.classifier(model.features(Variable(X).type(dtype))).shape)
